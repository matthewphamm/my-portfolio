export default function About() {
    return (
        <section id="about" className="max-w-4xl mx-auto px-6 py-20 border-t border-line">
            <p className="font-mono text-sm text-cobalt mb-4">// 01 - about</p>
            <p className="font-body text-m leading-relaxed max-w-2xl">
                I'm a Computer Science student concentrating in Artificial Intelligence. I'm consistently growing and learning, with hands-on experience
                in building multiple GitHub projects, including data science applications, machine learning models, and AI-powered applications. Currently, 
                I'm learning how AI agents can help my workflow and how AI api's can be implemented in future projects. 
                <br />
                <br />
                In my free time, I enjoy outdoor activities such as hiking, soccer, and running. I'm also an avid film enthusiast, drawn to the creativity, 
                cinematography, and storytelling that great films offer. 
            </p>
        </section>
    );
}